...
async def test_deep_maintenance_creates_candidate_synthesis_and_refreshes_storage(tmp_vault: Path):
    ...
    assert "## Topic Overview" in candidate_text
    assert "## Strongest Source Briefs" in candidate_text
    assert "## Agreements" in candidate_text
    assert "## Tensions / Missing Coverage" in candidate_text
    assert "## Recommended Next Step" in candidate_text
    assert candidate_meta["source_note_paths"] == [
        "wiki/sources/articles/agent-memory.md",
        "wiki/sources/articles/memory-planning.md",
    ]
    assert "agreement_points" in candidate_meta
    assert "tension_points" in candidate_meta
    assert candidate_meta["lifecycle"]["confidence"] is not None
